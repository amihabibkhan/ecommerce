                        ...
                        @foreach($categories as $single_item)
                            ...
                            <div id="custom-modal-{{ $single_item->id }}" class="modal-demo text-left">
                                ...
                                <div class="custom-modal-text">
                                    <form action="{{ route('manage_category.update', $single_item->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="form-group">
                                            <label for="">Parent Category</label>
                                            <select name="main_category_id" class="form-control">
                                                @foreach($main_categories as $single_main_item)
                                                    <option value="{{ $single_main_item->id }}" {{ $single_item->main_category_id == $single_main_item->id ? 'selected' : ''}}>{{ $single_main_item->title }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label for="">Category Name</label>
                                            <input type="text" class="form-control" name="title" value="{{ $single_item->title }}">
                                        </div>
                            ...
                            <div id="delete-modal-{{ $single_item->id }}" class="modal-demo text-left">
                                ...
                                <div class="custom-modal-text text-center">
                                    ...
                                    <form action="{{ route('manage_category.destroy', $single_item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <div class="text-center">
